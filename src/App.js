...
const App = () => {
  ...
  const fetchingData = async () => {
    try {
      const res = await fetch("https://keen-quick-airport.glitch.me/tasks");
  ...
  const deleteData = async (id) => {
    await fetch(`https://keen-quick-airport.glitch.me/tasks/${id}`, {
      method: "DELETE",
      headers: {
  ...
  const puttingRequest = async (id, newData) => {
    fetch(`https://keen-quick-airport.glitch.me/tasks/${id}`, {
      method: "PUT",
      headers: {
        "Content-type": "application/json",
      },
